...
** Si lo tiene https://["Credenciales de AWS"^] o. https://["Credenciales de Azure"^] En la cuenta, el catálogo indexado se almacena en el proveedor de cloud, al igual que con un conector puesto en marcha en el cloud. (Si tiene ambas credenciales, AWS está seleccionado de forma predeterminada.)
+
...
* Para los backups almacenados en AWS, aprovisiona un nuevo bloque de S3 y el https://["Servicio de consultas interactivas de Amazon Athena"^] y.. https://["Servicio de integración de datos sin servidor de AWS"^].
...
* Para los backups almacenados en Google Cloud, aprovisiona un nuevo bloque y el https://["Servicios de Google Cloud BigQuery"^] se aprovisionan en el nivel de cuenta/proyecto.
