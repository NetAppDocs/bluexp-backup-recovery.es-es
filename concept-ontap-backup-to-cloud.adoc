...
summary: El backup y la recuperación de datos de BlueXP ofrece funcionalidades de backup y restauración para proteger y archivar a largo plazo los datos de tu clúster de ONTAP. Los backups se generan y almacenan automáticamente en un almacén de objetos en su cuenta de cloud público o privado, independientemente de las copias Snapshot de volumen que se utilicen para la recuperación o el clonado a corto plazo. 
---
...
[role="lead"]
El backup y la recuperación de datos de BlueXP ofrece funcionalidades de backup y restauración para proteger y archivar a largo plazo los datos de tu clúster de ONTAP. Los backups se generan y almacenan automáticamente en un almacén de objetos en su cuenta de cloud público o privado, independientemente de las copias Snapshot de volumen que se utilicen para la recuperación o el clonado a corto plazo.
...
El backup y la recuperación de datos de BlueXP se pueden utilizar en un sitio sin conectividad a Internet (también conocido como sitios «en modo privado» o «oscuros») para hacer backups de los datos del volumen. En este caso, deberá desplegar el conector BlueXP en el sitio oscuro.
...
* Un plan de backup y recuperación de BlueXP que te permite realizar backups de los datos de Cloud Volumes ONTAP y de ONTAP on-premises.
...
* En este momento, EL modo WORM y cumplimiento de normativas en un almacén de objetos es compatible con Amazon S3, Azure y StorageGRID. Esto se conoce como la función DataLock y debe gestionarse mediante la configuración de backup y recuperación de BlueXP, no utilizando la interfaz del proveedor de la nube.
