...
. Copie el contenido de *sudoer.txt* ubicado en: _/var/lib/docker/volumes/service-manager[1]-2_cloudmanager_scs_cloud_volume/_data/$(sudo docker ps|grep -Po «|sed -e's/ *$/'|cut -f2 -d»:)/sc-linux-host-plugin_
...
(Opcional) Si no se agrega el host, haga clic en *Agregar host de plugin*.
